...
pub async fn handle_incoming_message(node: Arc<Mutex<Node>>, mining_flag: Arc<AtomicBool>, msg: IrohMessage) {
        ...
        Ok(parsed) => {
            match parsed {
                ...
                Message::BlockMined {from, block} => {
                    on_block_received(node, mining_flag, from, block).await;
                }
                _ => {
...
async fn on_block_received(node: Arc<Mutex<Node>>, mining_flag: Arc<AtomicBool>, from: NodeId, block: Block) {
